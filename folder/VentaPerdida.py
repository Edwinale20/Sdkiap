...
if venta_perdida_data is not None and not venta_pr_data.empty:
    ...
    # Aplicar filtros y mostrar dashboard
    ...
    with col1:
        ...
        total_venta_neta = combined_data['Venta Neta Total'].sum()
        total_venta_perdida_filtrada = filtered_data['VENTA_PERDIDA_PESOS'].sum()
        porcentaje_acumulado = (total_venta_perdida_filtrada / total_venta_neta) * 100
    ...
    with col2:
        ...
        st.plotly_chart(make_donut_chart(filtered_data['VENTA_PERDIDA_PESOS'].sum(), total_venta_neta, 'Acumulada', 'orange'), use_container_width=True)
