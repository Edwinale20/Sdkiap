...
def apply_weekly_view(data):
    ...
    weekly_data = weekly_data.groupby(['Semana', 'PROVEEDOR', 'PLAZA', 'CATEGORIA', 'DIVISION', 'DESC_ARTICULO']).agg({
...
# Function to plot venta perdida por día o semana
def plot_venta_perdida(data, view):
    fig = go.Figure()
    grouped_data = data.groupby('Fecha' if view == "diaria" else 'Semana')['VENTA_PERDIDA_PESOS'].sum().reset_index()
    fig.add_trace(go.Scatter(x=grouped_data['Fecha' if view == "diaria" else 'Semana'], y=grouped_data['VENTA_PERDIDA_PESOS'], mode='lines+markers', name='Venta Perdida', line=dict(color='rgb(219, 64, 82)')))
    fig.update_layout(title=f'Venta Perdida {view.capitalize()}', xaxis_title='Fecha' if view == "diaria" else 'Semana', yaxis_title='Monto (Pesos)', yaxis=dict(tickformat="$,d"))
    return fig
...
def plot_venta_perdida_con_tendencia(data, view):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=grouped_data['Fecha' if view == "diaria" else 'Semana'], y=grouped_data['VENTA_PERDIDA_PESOS'], name='Venta Perdida', marker_color='rgb(219, 64, 82)'))
    fig.add_trace(go.Scatter(x=grouped_data['Fecha' if view == "diaria" else 'Semana'], y=grouped_data['Cambio (%)'], mode='lines+markers', name='Cambio Porcentual', line=dict(color='white'), yaxis='y2'))
    fig.update_layout(title=f'Venta Perdida por {view.capitalize()} y Cambio Porcentual', xaxis_title='Fecha' if view == "diaria" else 'Semana', yaxis=dict(title='Monto (Pesos)', tickformat="$,d"), yaxis2=dict(title='Cambio Porcentual (%)', overlaying='y', side='right', tickformat=".2f", showgrid=False), legend=dict(x=0, y=1.1, orientation='h'), barmode='group')
    return fig

# Function to plot venta perdida por proveedor
def plot_venta_perdida_proveedor(data, view, selected_proveedor=None):
    ...
    fig.update_layout(title=f'Venta Perdida por Proveedor ({view})')
    return fig
...
# Function to plot venta perdida vs venta neta total diaria o semanal
def plot_comparacion_venta_perdida_vs_neta_diaria(data, venta_pr_data, filtro_fechas, view, view_percentage=False):
    filtered_venta_pr = venta_pr_data[venta_pr_data['Día Contable'].isin(filtro_fechas)]
    comparacion_diaria = data.groupby('Fecha' if view == "diaria" else 'Semana')['VENTA_PERDIDA_PESOS'].sum().reset_index()
    comparacion_diaria = comparacion_diaria.merge(filtered_venta_pr.groupby('Día Contable' if view == "diaria" else 'Semana')['Venta Neta Total'].sum().reset_index(), left_on='Fecha' if view == "diaria" else 'Semana', right_on='Día Contable' if view == "diaria" else 'Semana')
    if view_percentage:
        ...
        fig = go.Figure(data=[go.Bar(name='Venta Perdida (%)', x=comparacion_diaria['Fecha' if view == "diaria" else 'Semana'], y=comparacion_diaria['Venta Perdida (%)'], marker_color='red'), go.Bar(name='Venta Neta Total (%)', x=comparacion_diaria['Fecha' if view == "diaria" else 'Semana'], y=comparacion_diaria['Venta Neta Total (%)'], marker_color='blue')])
        fig.update_layout(barmode='stack', title=f'Venta Perdida vs Venta Neta Total ({view}) (Porcentaje)', xaxis_title='Fecha' if view == "diaria" else 'Semana', yaxis_title='Porcentaje (%)')
    else:
        fig = go.Figure(data=[go.Bar(name='Venta Perdida', x=comparacion_diaria['Fecha' if view == "diaria" else 'Semana'], y=comparacion_diaria['VENTA_PERDIDA_PESOS'], marker_color='red'), go.Bar(name='Venta Neta Total', x=comparacion_diaria['Fecha' if view == "diaria" else 'Semana'], y=comparacion_diaria['Venta Neta Total'], marker_color='blue')])
        fig.update_layout(barmode='stack', title=f'Venta Perdida vs Venta Neta Total ({view})', xaxis_title='Fecha' if view == "diaria" else 'Semana', yaxis_title='Monto (Pesos)', yaxis=dict(tickformat="$,d"))
    return fig
...
def plot_venta_perdida_mercado(data, view):
    ...
    for mercado in mercados:
        ...
        grouped_data = mercado_data.groupby('Fecha' if view == "diaria" else 'Semana')['VENTA_PERDIDA_PESOS'].sum().reset_index()
        fig.add_trace(go.Scatter(x=grouped_data['Fecha' if view == "diaria" else 'Semana'], y=grouped_data['VENTA_PERDIDA_PESOS'], mode='lines+markers', name=mercado))
    fig.update_layout(title=f'Venta Perdida por {view.capitalize()} y por Mercado', xaxis_title='Fecha' if view == "diaria" else 'Semana', yaxis_title='Venta Perdida (Pesos)', yaxis=dict(tickformat="$,d"))
    return fig
...
if data is not None:
    ...
    vista = st.sidebar.radio("Selecciona la vista:", ("Diaria", "Semanal"))
    filtered_data = apply_filters(data, proveedores, plaza, categoria, None, semana_seleccionada, division, articulo)
    weekly_data = apply_weekly_view(filtered_data) if vista == "Semanal" else filtered_data
    ...
    with col1:
        ...
        comparacion_diaria = weekly_data.groupby('Fecha' if vista == "diaria" else 'Semana')['VENTA_PERDIDA_PESOS'].sum().reset_index()
        comparacion_diaria = comparacion_diaria.merge(venta_pr_data.groupby('Día Contable' if vista == "diaria" else 'Semana')['Venta Neta Total'].sum().reset_index(), left_on='Fecha' if vista == "diaria" else 'Semana', right_on='Día Contable' if vista == "diaria" else 'Semana')
        ...
        st.markdown(f'#### Venta Perdida {vista}')
        st.plotly_chart(plot_venta_perdida(weekly_data, vista), use_container_width=True)
    with col2:
        st.markdown(f'#### Venta Perdida {vista} 📅')
        st.plotly_chart(make_donut_chart(weekly_data['VENTA_PERDIDA_PESOS'].sum(), total_venta_perdida, vista.capitalize(), 'orange'), use_container_width=True)
    col3, col4 = st.columns((1, 1))
    with col3:
        st.markdown(f'#### Venta Perdida vs Venta Neta Total ({vista})')
        st.plotly_chart(plot_comparacion_venta_perdida_vs_neta(weekly_data, venta_pr_data, weekly_data['Fecha' if vista == "diaria" else 'Semana'], vista), use_container_width=True)
    with col4:
        st.markdown(f'#### Venta Perdida por Plaza ({vista})')
        st.plotly_chart(plot_venta_perdida_plaza(weekly_data, vista), use_container_width=True)
    col5, col6 = st.columns((1, 1))
    with col5:
        st.markdown(f'#### Top 10 Artículos con Mayor Venta Perdida ({vista})')
        st.plotly_chart(plot_articulos_venta_perdida(weekly_data), use_container_width=True)
    with col6:
        st.markdown(f'#### Venta Perdida por Proveedor ({vista})')
        st.plotly_chart(plot_venta_perdida_proveedor(weekly_data, vista, proveedores), use_container_width=True)
    col7, col8 = st.columns((1, 1))
    with col7:
        st.markdown(f'#### Cambio porcentual de venta perdida ({vista})')
        st.plotly_chart(plot_venta_perdida_con_tendencia(weekly_data, vista), use_container_width=True)
    with col8:
        st.markdown(f'#### Venta Perdida vs Venta Neta Total ({vista})')
        st.plotly_chart(plot_comparacion_venta_perdida_vs_neta_diaria(weekly_data, venta_pr_data, weekly_data['Fecha' if vista == "diaria" else 'Semana'], vista), use_container_width=True)
    st.markdown(f'#### Venta Perdida diaria por Mercado ({vista})')
    st.plotly_chart(plot_venta_perdida_mercado(weekly_data, vista), use_container_width=True)
else:
